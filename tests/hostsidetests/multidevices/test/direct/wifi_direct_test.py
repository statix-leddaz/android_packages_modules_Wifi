...
class WifiDirectTest(base_test.BaseTestClass):
  ...
  def _setup_device(self, ad: android_device.AndroidDevice) -> None:
    ...
    ad.adb.install([
        ...
        '-g',
        '-r',
        self.user_params[tag]['wifi_direct_mobly_snippet'][0],
    ])
  ...
  def test_wifi_direct_connect(self) -> None:
    ...
    try:
      ...
      failure_event = owner_action_handler.waitAndGet('onFailure')
      failure_reason = constants.ActionListenerOnFailure(
          failure_event.data['reason']
      )
    ...
    try:
      ...
      failure_event = client_action_callback.waitAndGet('onFailure')
      failure_reason = constants.ActionListenerOnFailure(
          failure_event.data['reason']
      )
      asserts.fail(
